...
member_grade varchar2(20) default '일반'
);
...
member_addr_status varchar2(15) not null check(member_addr_status in('기본주소', '추가주소')),
...
custom_order_type varchar2(15) not null check(custom_order_type in('요청서', '견적서')) 
...
seller_grade varchar2(20) default '일반',
...
---------------------------------------------------- 주문상세내용-옵션
-- 외래키 ordering_info -> info (이름이 너무 길어서..)
